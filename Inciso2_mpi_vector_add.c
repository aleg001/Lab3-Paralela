...
int main(void)
{
    ...
    Print_vector(local_x, local_n, n, "El Vector x es: ", my_rank, comm);
    Print_vector(local_y, local_n, n, "El Vector y es: ", my_rank, comm);
    ...
    MPI_Comm comm /* in  */)
{
    *n_p = 100000;
    ...
    {
        fprintf(stderr, "Order should be > 0 and divisible by comm_sz\n");
        MPI_Abort(comm, -1);
    }
    ...
    MPI_Comm comm /* in  */)
{
    ...
    if (my_rank == 0)
    {
        ...
        for (i = 0; i < n; i++)
            a[i] = rand() % 100; // Genera valores aleatorios entre 0 y 99
    ...
    MPI_Comm comm /* in */)
{
    ...
    if (my_rank == 0)
    {
        ...
        printf("%s\n", title);
        for (i = 0; i < 10; i++) // Imprime los primeros 10 elementos
            printf("%f ", b[i]);
        printf("... ");
        for (i = n - 10; i < n; i++) // Imprime los últimos 10 elementos
            printf("%f ", b[i]);
        printf("\n");
        free(b);
    }
