...
int main(void)
{
    ...
    // Leer el escalar desde el proceso 0 y transmitirlo a todos los procesos
    double scalar;
    if (my_rank == 0)
    {
    ...
    // Calcular el producto de un escalar por cada vector
    Parallel_scalar_product(local_x, local_y, scalar, local_n);

    // Opcionalmente, imprimir los vectores resultantes
...
{
    double local_dot = 0.0, dot;
    for (int i = 0; i < local_n; i++)
        local_dot += local_x[i] * local_y[i];
    MPI_Allreduce(&local_dot, &dot, 1, MPI_DOUBLE, MPI_SUM, comm);
    return dot;
}
